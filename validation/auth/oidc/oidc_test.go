...
import (
	...
	features "github.com/rancher/shepherd/extensions/kubeapi/features"
...
func (s *OIDCTestSuite) SetupSuite() {
	...
	err = deployments.WaitForDeploymentActive(client, "local", deployments.RancherDeploymentNamespace, deployments.RancherDeploymentName)
	...
	err = oidcclient.CreateOIDCClient(client, s.oidcConfig.ClientName, spec)
	require.NoError(s.T(), err, "Failed to create OIDCClient CRD")
...
// craftJWTWithIntKid returns a malformed JWT whose header `kid` field is an integer.
...
func (s *OIDCTestSuite) TestTokenWhenFeatureFlagDisabled() {
	...
	err = deployments.WaitForDeploymentActive(s.client, "local", deployments.RancherDeploymentNamespace, deployments.RancherDeploymentName)
